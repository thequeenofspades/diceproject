...
def get_positive_features(train_path_pos, cell_size, window_size, block_size, nbins):
    ...
    image_files = [os.path.join(train_path_pos, f.strip()) for f in trainfile]
    trainfile.close()
    ...
    for i in range(num_images):
        ...
        print "Computing positive features for %s" % (image_files[i])
...
def get_random_negative_features(non_face_scn_path, cell_size, window_size, block_size, nbins, num_samples):
    ...
    for i in range(num_images):
        print "Computing negative features for %s" % image_files[i]
        #img = np.mean(imread(image_files[i]), 2).astype(np.uint8)
        img = Image.open(image_files[i])
        img.load()
        img = img.convert('L')
        if img.height < window_size[0] or img.width < window_size[1]:
            print "Resizing image from %d x %d" % (img.width, img.height)
            ratio = max(1., window_size[0] / float(min(img.height, img.width)))
            img = img.resize((int(ratio*img.width), int(ratio*img.height)))
        img = np.array(img)
        height, width = img.shape
        print "Dimensions: %d x %d" % (height, width)
